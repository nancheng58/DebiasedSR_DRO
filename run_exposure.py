...
def main():
    ...
    _, _, item_counter = get_user_seqs(args.data_file + "/ori_Exposure_Train.txt")
    train_data, max_item, _ = get_user_seqs(args.data_file + "/Exposure_Train.txt")  # Exposure_Train
    item_size = max(item_size, max_item)
    valid_data, max_item, _ = get_user_seqs(args.data_file + "/Exposure_Valid.txt")
    item_size = max(item_size, max_item)
    test_data, max_item, _ = get_user_seqs(args.data_file + "/Exposure_Test.txt")
    item_size = max(item_size, max_item)
    args.item_size = item_size + 2
    if args.data_name == "tenrec_5core":
        args.item_size += 1
    ...
    # save model args
    args_str = f'{args.exposure_model_name}-{args.data_name}-Exposure'
